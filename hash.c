...
void destruir_listas(lista_t** listas, size_t i, hash_t *hash){
	for(size_t n = 0 ;n <= i ; n++){
		while(!lista_esta_vacia(listas[n])){
			campo_t* campo = (campo_t*)lista_borrar_primero(listas[n]);
			campo_destruir(campo,hash->destruir_dato);
		}
		lista_destruir(listas[n],NULL);
	}
	free(listas);
}
...
bool hash_redimensionar(hash_t* hash, int redimension ){
	...
	for(size_t i = 0; i<TAM_NUEVO ; i++){
		...
		if(!listas_nuevo[i]){
			destruir_listas(listas_nuevo,i,hash);
			//free(listas_nuevo);	
			return false;
		}
	}
...
hash_t *hash_crear(hash_destruir_dato_t destruir_dato){
	...
	for(int i = 0; i<TAM ; i++){
		...
		if(!hash->listas[i]){
			destruir_listas(hash->listas,i,hash);
			free(hash);	
			return NULL;
		}
	}	
